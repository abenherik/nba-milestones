...
export default async function PointsBeforeAgePage({ searchParams }: { searchParams: { age?: string; includePlayoffs?: string } }) {
  ...
  const data = await getBeforeAgeSqlite('points', age, includePlayoffs);


  return (
    <main className="max-w-4xl mx-auto p-4">
      <PageHeader
        title={`Points before age ${age}`}
        right={<MetricTabs current="points" age={age} includePlayoffs={includePlayoffs} />}
      />

      <Card className="mb-4 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <AgePills age={age} includePlayoffs={includePlayoffs} basePath="/leaderboards/points-before-age" />
          <form className="flex items-center gap-3">
            ...
            <button className="px-2 py-1" formAction="/leaderboards/points-before-age">Apply</button>
          </form>
        </div>
      </Card>
