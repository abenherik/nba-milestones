...
import { BeforeAgeRow } from '../../../lib/leaderboards/beforeAgeSqlite';
...
function BlocksBeforeAgeContent() {
  const searchParams = useSearchParams();
  const [data, setData] = useState<{ top25: BeforeAgeRow[]; definition?: string; updatedAt?: string } | null>(null);
  const [loading, setLoading] = useState(true);

  const age = Number(searchParams?.get('age') ?? 21);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/leaderboards/blocks-before-age?${params}`);
        if (res.ok) {
  ...
  return (
    <main className="max-w-4xl mx-auto p-4">
      <PageHeader
        title={`Blocks before age ${age}`}
        right={<MetricTabs current="blocks" age={age} includePlayoffs={includePlayoffs} />}
      />

      <Card className="mb-4 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <AgePills age={age} includePlayoffs={includePlayoffs} basePath="/leaderboards/blocks-before-age" />
          <form className="flex items-center gap-3">
            ...
            <button className="px-2 py-1" formAction="/leaderboards/blocks-before-age">Apply</button>
          </form>
        </div>
      </Card>
      ...
      {!loading && !data && <p className="text-red-600">Leaderboard not found</p>}
...
export default function BlocksBeforeAgePage() {
  return (
    <Suspense fallback={<div className="max-w-4xl mx-auto p-4"><p className="text-gray-600">Loading...</p></div>}>
      <BlocksBeforeAgeContent />
    </Suspense>
  );
}
