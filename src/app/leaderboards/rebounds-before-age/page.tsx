...
export default async function ReboundsBeforeAgePage({ searchParams }: { searchParams: { age?: string; includePlayoffs?: string } }) {
  ...
  return (
    <main className="max-w-4xl mx-auto p-4">
      <PageHeader
        title={`Rebounds before age ${age}`}
        right={<MetricTabs current="rebounds" age={age} includePlayoffs={includePlayoffs} />}
      />

      <Card className="mb-4 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <AgePills age={age} includePlayoffs={includePlayoffs} basePath="/leaderboards/rebounds-before-age" />
          <form className="flex items-center gap-3">
            ...
            <button className="px-2 py-1" formAction="/leaderboards/rebounds-before-age">Apply</button>
          </form>
        </div>
      </Card>
