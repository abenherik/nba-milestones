...
function jitter(ms: number) { const j = Math.floor(Math.random() * Math.min(400, Math.max(50, ms * 0.4))); return ms + j; }
...
async function upsertGames(db: ReturnType<typeof openSqlite>, playerId: string, season: string, seasonType: SeasonType, games: any[]) {
  for (const g of games) {
    const gameId = String(g.GAME_ID ?? g.game_id ?? `${playerId}:${season}:${g.GAME_DATE}`);
...
async function hasExistingSeason(db: ReturnType<typeof openSqlite>, playerId: string, season: string, seasonType: SeasonType) {
  const rows = await dbAll<{ c: number }>(db, `SELECT COUNT(*) as c FROM player_stats WHERE player_id = ? AND season = ? AND season_type = ? LIMIT 1`, [playerId, season, seasonType]);
  return (rows?.[0]?.c ?? 0) > 0;
}
...
async function main() {
  ...
  const START_AT = String(process.env.START_AT || ''); // resume by player id
  const SKIP_EXISTING = String(process.env.SKIP_EXISTING || '1') === '1';

  const players = await dbAll<{ id: string; full_name: string }>(db,
    `SELECT id, full_name FROM players WHERE is_active = 1 ORDER BY id`
  );

  let started = START_AT ? false : true;
  let fetched = 0;
  for (const p of players) {
    if (!started) {
      if (p.id === START_AT) started = true;
      else continue;
    }
    if (LIMIT && fetched >= LIMIT) break;

    let seasons = await getSeasonsForPlayer(p.id);
    ...
    for (const s of seasons) {
      for (const t of types) {
        try {
          if (SKIP_EXISTING && await hasExistingSeason(db, p.id, s, t)) {
            continue;
          }
          const games = await fetchPlayerGameLog(p.id, s, t);
          await upsertGames(db, p.id, s, t, games);
          await sleep(jitter(DELAY_MS));
        } catch (e) {
          console.warn(`Failed ${p.id} ${p.full_name} ${s} ${t}:`, (e as Error).message);
        }
      }
    }
    fetched++;
    console.log(`Fetched ${p.full_name} (${p.id}) — ${fetched}/${LIMIT || players.length}`);
    await sleep(jitter(Math.max(500, DELAY_MS)));
  }
