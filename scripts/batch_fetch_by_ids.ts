...
async function main() {
  ...
  const players = await dbAll<{ id: string; full_name: string }>(db,
    `SELECT id, full_name FROM players WHERE id IN (${ids.map(() => '?').join(',')}) ORDER BY full_name`, ids
  );
  ...
  for (const p of players) {
    ...
    for (const s of seasons) {
      for (const t of types) {
        try {
          if (SKIP_EXISTING) {
            const row = await dbAll<{ c: number }>(db, `SELECT COUNT(*) as c FROM player_stats WHERE player_id = ? AND season = ? AND season_type = ?`, [p.id, s, t]);
            if ((row?.[0]?.c ?? 0) > 0) continue;
          }
    ...
    console.log(`Fetched ${p.full_name} (${p.id})`);
